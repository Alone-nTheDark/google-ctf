...
impl Item {
    pub fn new(
        ...
        let mut sprite = items::sprite_init_fn(item_type)(
    ...
    pub fn update(ctx: &mut Ctx, item_id: usize) -> (Option<u16>, Option<ItemType>) {
        let item = &mut ctx.map.items[item_id];
        if item.collected {
            return (None, None);
        }
        ...
        if item.hitbox().collides(&ctx.player.hitbox()) {
            item.collected = true;
            if item.apply_immediate_effects(&mut ctx.player) {
