...
pub const DIALOGUES: &[fn(&mut Ctx)] = &[
    example_dialogue, // 0
    bark_bark,        // 1
];
...
impl Npc {
    pub fn new(
        ...
        let mut sprite = npcs::sprite_init_fn(npc_type)(
            res_state, vdp, /* singleton= */ false, /* keep_loaded= */ false,
        );
        sprite.set_position(sprite_x, sprite_y);
        sprite.set_animation(Sprite::Anim::Idle as usize);
        Npc {
            x: sprite_x,
            y: sprite_y,
            id: properties.id,
    ...
    pub fn update(ctx: &mut Ctx, npc_id: usize) {
        ...
        {
            let npc = &mut ctx.map.npcs[npc_id];
            let hitbox = npc.hitbox();
            let player_hitbox = &mut ctx.player.hitbox();
            ...
            if let Some(input) = input {
                ...
                if input.just_pressed(Button::B) && player_hitbox.expand(5).collides(&hitbox) {
                    ...
                    npc.sprite.maybe_set_animation(Sprite::Anim::Idle as usize);
                    npc.sprite.flip_h = dx >= 0;
                }
            }
        }
        if let Some(dialogue_id) = dialogue_id {
            DIALOGUES[dialogue_id as usize](ctx);
        }
    }
...
fn example_dialogue(ctx: &mut Ctx) {
...
fn bark_bark(ctx: &mut Ctx) {
    ctx.start_dialogue(Dialogue::new_no_response("BARK BARK!", None));
}
