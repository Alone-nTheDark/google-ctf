...
/// The fixed-width string type all dialogues return as the player response.
pub type ResponseStr = String<{ TEXT_WIDTH as usize }>;
...
impl Dialogue {
    ...
    /// (the player can write text from a set of allowed characters).
    ...
    pub fn update(ctx: &mut Ctx) -> Option<ResponseStr> {
        ...
        let dialogue = ctx.dialogue.as_mut().unwrap();
        ...
        if dialogue.render_state.text_complete {
            ...
            if b_pressed {
                ...
                return match &dialogue.response {
                    ...
                    ResponseType::FreeText { text, .. } => Some(text.clone()),
                };
            }
        } else if b_pressed {
    ...
    pub fn clear(vdp: &mut TargetVdp) {
        for x in 0..TEXT_WIDTH + 2 {
            for y in 0..TEXT_HEIGHT + 2 {
                Image::clear_tile(TEXT_START_X + x - 1, TEXT_START_Y + y - 1, vdp, Plane::B);
            }
        }
    }
        ...
        match &self.response {
            ...
            ResponseType::MultipleChoice { choices, .. } => {
                for c in 0..choices.len() {
                    ...
                    UI::draw_text(&choices[c], x, y, &ui.inventory_text_img, vdp, Plane::B);
                }
                // Draw choice arrow.
                let (x, y) = Self::get_choice_text_pos(choices, 0);
                Image::draw(&ui.choice_arrow_img, x - 1, y, vdp, Plane::B);
            }
    ...
    fn get_choice_text_pos(choices: &Vec<ResponseStr, 8>, choice_idx: u16) -> (u16, u16) {
    ...
    fn update_response(
        ...
        match &mut self.response {
            ...
            ResponseType::MultipleChoice { choices, selection } => {
                ...
                Image::draw(&ui.choice_arrow_img, new_x - 1, new_y, vdp, Plane::B);
            }
            ResponseType::FreeText { text, charset } => {
                ...
                if up_pressed || down_pressed {
                    ...
                    UI::draw_text_char(new_chr as u8, x, y, &ui.inventory_text_img, vdp, Plane::B);
                } else if right_pressed && text.len() < TEXT_WIDTH as usize {
                    ...
                    Image::draw_tile(&ui.inventory_border_img, 7, x, y + 1, vdp, Plane::B);
                    // Add new arrow.
                    Image::draw(&ui.text_arrow_img, x + 1, y + 1, vdp, Plane::B);
                } else if left_pressed && text.len() > 1 {
                    // Delete current char.
                    text.pop();
                    UI::draw_text("_", x, y, &ui.inventory_text_img, vdp, Plane::B);
                    // Clear previous arrow.
                    Image::draw_tile(&ui.inventory_border_img, 7, x, y + 1, vdp, Plane::B);
                    // Add new arrow.
                    Image::draw(&ui.text_arrow_img, x - 1, y + 1, vdp, Plane::B);
