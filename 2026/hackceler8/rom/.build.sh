...
m68k-elf-objcopy -O binary -j .text -j .rodata -j .data /tmp/out.elf /src/hx8.md

if [[ -n "$HOST_UID" && -n "$HOST_GID" ]]; then
  chown -R "$HOST_UID:$HOST_GID" /src/game/src/res /src/.cargo_home /src/target.docker /src/hx8.elf /src/hx8.md 2>/dev/null || true
fi
