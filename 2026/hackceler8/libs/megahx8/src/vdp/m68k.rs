...
/// Length of a dma transfer (= number of words).
struct DmaLength(u16);

impl DmaLength {
    fn from_slice<T>(s: &[T]) -> Self {
        Self((size_of_val(s) >> 1) as u16)
    }
}
...
impl Vdp {
    ...
    ///
    /// This will be incremented after every write via [`self.write_data`].
    ...
    fn dma_upload(
        ...
        dst_addr: u16,
        src_addr: *const (),
        mut length: DmaLength,
    ) {
        assert_eq!(src_addr as u32 % 2, 0, "Misaligned DMA");
        let mut src_addr = src_addr as u32;
        let mut dst_addr = u32::from(dst_addr);
        assert!(
            (src_addr >> 1) <= 0x7F_FFFF,
            "Source address unreachable for DMA"
        );

        self.enable_dma(true);
        while length.0 > 0 {
            // See https://md.railgun.works/index.php?title=VDP#DMA
            // DMA transfers must not cross 128KB boundary.
            let max_transfer_bytes = 0x20000 - (0x1ffff & src_addr);
            let dma_length_words = (max_transfer_bytes / 2).min(length.0 as u32);
            let dma_length_words = dma_length_words as u16;

            self.set_register(registers::DMA_LEN_L, dma_length_words as u8);
            self.set_register(registers::DMA_LEN_H, (dma_length_words >> 8) as u8);
            self.set_register(registers::DMA_SRC_L, (src_addr >> 1) as u8);
            self.set_register(registers::DMA_SRC_M, (src_addr >> 9) as u8);
            self.set_register(registers::DMA_SRC_H, (src_addr >> 17) as u8);
            self.set_addr_raw(kind, dst_addr as u16, /*dma=*/ true);
            self.wait_for_dma();

            dst_addr += u32::from(dma_length_words) * 2;
            src_addr += u32::from(dma_length_words) * 2;
            length.0 -= dma_length_words;
        }
