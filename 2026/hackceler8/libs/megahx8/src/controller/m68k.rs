...
use crate::portal::CARTRIDGE_ID_VALUE;
...
// This function will compile to a single `nop` instruction followed by a `ret`.
fn nop() {
    unsafe {
...
impl Controllers {
    ...
    pub(crate) fn new(enable_six_buttons: bool) -> Controllers {
        ...
        if running_on_console() {
            ...
            return Controllers {
                ...
                enable_six_buttons: enable_six_buttons,
            };
        }
...
impl super::Controllers for Controllers {
    ...
    fn update(&mut self) {
        ...
        let (mut c1_ext1, mut c2_ext1) = (0, 0);
        if self.enable_six_buttons {
            Controllers::read_pins();
            (c1_ext1, c2_ext1) = Controllers::read_pins();
        }
...
fn running_on_console() -> bool {
    unsafe { read_volatile(CARTRIDGE_HARDWARE_PRESENT) == CARTRIDGE_ID_VALUE }
}
