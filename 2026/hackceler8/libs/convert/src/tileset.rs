...
pub fn load_png(file: impl AsRef<Path> + std::fmt::Debug) -> std::io::Result<(Palette, Vec<u8>)> {
    ...
    // 3 bytes per color
    if palette.len() > 16 * 3 {
        panic!("Image uses too many colors: {:?} {}", file, palette.len());
    }
...
pub fn convert_tileset(
    tsx_file: impl AsRef<Path> + std::fmt::Debug + Copy,
    ...
    eprintln!("try_apply {:?} {:?}", tsx_file, palette_data);
...
mod tests {
    ...
    fn png_loading_works() {
        let result = load_png("../../resources/sprites/player/player.png");
        assert!(result.is_ok());
        let result = result.unwrap();
        assert!(result.0.colors[0].is_some());
        assert!(result.0.colors.iter().any(|c| c.is_some()));
    }
    ...
    fn conversion_to_tiles_works() {
        ...
        let tiles = image_to_tiles(&result.1, 20, 54, None);
        assert_eq!(tiles.len(), 20 * 54 * 8 * 4);
    }
}
