...
pub struct PaletteID {
    id: GamePalette,
    /// The applicable context to load this palette in, e.g.
    /// the map ("water-temple") or a menu like "stage-select".
    /// If not set, this is the default context.
...
pub enum GamePalette {
    ...
    UI,
    /// Palette of all enemies. This is map-specific
    Enemy,
    /// Palette of the map background and tiles. This is map-specific
    Background,
}

impl GamePalette {
    pub fn for_context(self, context: &str) -> PaletteID {
        PaletteID {
            id: self,
...
pub(crate) struct PaletteData {
    /// Map of palette IDs to their possible values.
    palettes: BTreeMap<char, PaletteValues>,

    /// List of all palette contexts.
    contexts: BTreeSet<String>,
}

#[derive(serde_derive::Serialize)]
pub(crate) struct PaletteValues {
    /// Palettes for a given context (e.g. the loaded map).
    context_specific: BTreeMap<String, [u16; 16]>,
    /// Default palette chosen when no particular context is active.
    default: [u16; 16],
}
...
impl Converter {
    ...
    pub fn convert_map(&mut self, map: &Map) -> Result<(), Box<dyn std::error::Error>> {
        let mut output_file = self.output_directory.clone();
        output_file.push("maps");
        std::fs::create_dir_all(&output_file)?;

        let mut map = map.clone();
        let map_id = map.identifier.clone();
        let palette_id = &GamePalette::Background.for_context(&map_id);

        let res = convert_map(self, &mut map, palette_id)?;
    ...
    pub fn write_palettes(
        ...
        palette_data.palettes = BTreeMap::new();
        palette_data.contexts = BTreeSet::new();
        for (key, palette) in self.palettes.iter() {
            ...
            if !palette_data.palettes.contains_key(&char::from(key.id)) {
                palette_data.palettes.insert(
                    char::from(key.id),
                    PaletteValues {
                        context_specific: BTreeMap::new(),
                        default: [0u16; 16],
                    },
                );
            }
            ...
            if let Some(context) = &key.context {
                let context = to_rust_enum_identifier(&context);
                palette_data.contexts.insert(context.to_string());
                palette_data
                    .palettes
                    .get_mut(&char::from(key.id))
                    .unwrap()
                    .context_specific
                    .insert(context.to_string(), raw_palette);
            } else {
                palette_data
                    .palettes
                    .get_mut(&char::from(key.id))
